...
import { b2Body, b2World, b2Contact, b2ContactImpulse, b2ContactListener, b2Fixture, b2Manifold, b2ParticleBodyContact, b2ParticleContact, b2ParticleSystem, b2Shape, XY } from "@flyover/box2d";
...
export type PhysicsTransform = {
    ...
    y: number;
    angle: number;
    updateTransform: (x: number, y: number, angle: number) => void;
}

export interface IBodyUserData<T extends PhysicsTransform> {
    label: string;
    gameObject: T;
}
 ...
 * @usage Add bodies
 * 
 * Schedules bodies to be created when physics engine is not calculating collisions
 * ```ts
 *       class Player {
 *           createPhysics(physicsSystem: PhysicsSystem<Piece>, bodyCreatedCallback: () => void) {
 *               physicsSystem.scheduleCreateBody(() => {
 *                    const bodyDef = // ...
 *                    physicsSystem.world.CreateBody(bodyDef);
 *               });
 *           }
 *          
 ...
 *           }
 *       }
 *       player.createPhysics(physicsSystem, function onBodyCreated() {
 *            console.log('Created');
 *       });
 ...
 * ```
 */
export class PhysicsSystem<T extends PhysicsTransform> implements b2ContactListener {
    ...
    readStateFromGame() {
        ...
        //     const userData: IBodyUserData<T> = body.GetUserData(); // Done: make an interface for user data
        //     const gameObject: T = userData?.gameObject ?? null;
    ...
    writeStateIntoGame() {
        const verboseLogs: string[] = [];
        for (let body = this.world.GetBodyList(); body; body = body.GetNext()) {
            const userData: IBodyUserData<T> = body.GetUserData();
            const gameObject: T = userData?.gameObject ?? null;
            const label = userData?.label ?? '(no label)';
            const name = (gameObject as any)?.name ?? '(no name)';
